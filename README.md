...
## Four things it is actually for
...
```text
  ...
  ────────────────────────────────────────────────────────────────────────────
    ...
    1 of 1 files have a recorded origin.
```

Two claims, and the report never makes them compete. The URL says how the file
reached this machine; the EXIF describes a life it had before that. Ranking them
would mean the download record deleting the GPS fix, the capture time and the
body serial number - which for an investigation are usually the valuable half.

Note `GPSDateStamp`: the satellite fix says the 23rd and the camera clock says
the 22nd. The GPS timestamp does not come from the camera's clock, so where the
two disagree the fix is the one to trust.

Across a folder, `--identify` rolls every coordinate, address and email up into
one list with the file and field each came from.
...
```text
  ...
  intrinsic  what the file records about its own earlier life
    ...
    document metadata   Adobe PDF Library 15.0                   self-reported
  ...
  reconciliation  conflict
    ...
    timeline_conflict   the file reports being created at
                        2026-08-24T09:00:00Z, after it arrived at
                        2026-08-10T19:02:11Z

  ────────────────────────────────────────────────────────────────────────────

  conclusion

    The acquisition records do not agree. The file may have been downloaded
    more than once, copied after it arrived, or had its origin metadata
    replaced - and nothing here settles which.

    The file describes an earlier life of its own - Adobe PDF Library 15.0 -
    which says nothing about how it arrived and does not contest the record
    above.

    The file also reports being created after it arrived, which cannot both
    be true. Either a clock was wrong, or the metadata was written after the
    fact.
```
...
its highest-scoring record - which is why a disagreement pulls every acquisition
record onto the screen without being asked.

### Two files that might be the same job

Two images turn up in different places and you want to know whether they are
related, and how each one travelled.
...
```text
  ...
  assessment
    ...
    Both files agree on Make, Model, BodySerialNumber, Software, but they
    arrived by different routes. Files that share an earlier life and not an
    acquisition path travelled separately, which is usually the more
    interesting of the two findings.
```

The body serial number ties both frames to one physical camera; fourteen seconds
apart makes them one burst. But only one of them has an acquisition record - the
other was opened here by an application, which proves contact and not arrival.
Shared earlier life without a shared route is usually the more interesting of the
two findings.
...
```text
  ...
  C2PA signature check       unavailable
                               manifests are read; validating the certificate
                               chain needs a crypto library
...
That last line is the honest limit: Chromium prunes at about ninety days, so a
file older than the oldest surviving record was never going to resolve.
...
| Recent documents | the application that opened the file, and when | 35 | Read from the freedesktop `recently-used.xbel` that every GTK application writes. The graphical equivalent of shell history: it proves something handled the file, not that it fetched it. |
